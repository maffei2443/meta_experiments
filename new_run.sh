...
mkdir new_experiments_outputs -p
...
#   echo $i-ninc
#   echo start: $(date "+%y%m%d.%H%M%S.%3N")
#   python metastream/ms_nonincremental_new.py --omega 300 --gamma 20 --initial 300 --target class --eval_metric acc --path data/${i}_ninc/ > $i-ninc.output
#   echo stop:  $(date "+%y%m%d.%H%M%S.%3N")
# done
